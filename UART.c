...
uint8_t string_buffer[BufferSize];
...
void USART_Start_Proc(int val){
  int n = sprintf((char *)string_buffer, "Lower:%d  Upper:%d  microseconds\r\n", val, val+100);
  USART_Write(USART2, (uint8_t *)string_buffer, n + 18);
}
...
/*
void UART_BOUNDS(int val){
  int nDigits;
  val = USART_Read(USART2);
  nDigits = floor(log10(abs(val)))+1;
  //char str[] = "Lower Bound: 50\r\n";
  USART_Write(USART2, (uint8_t*)"Lower Bound: 50\r\n", 19);
  USART_Write(USART2, (uint8_t*)"Upper Bound: 150\r\n", 19);
}*/
